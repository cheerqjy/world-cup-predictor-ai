...
- 赛事页面按日期新增未来/历史子标签
- 更新日志页面始终显示网盘下载入口
- 版本检查改为读取静态网盘链接，无需每次改配置
- 修复deploy.js文件占用导致打包失败的问题

### v1.6.0 (2026-06-26)
- 简化数据抓取模块：移除5个失效外部数据源，仅保留 worldcup26.ir API
- 重写数据同步逻辑：移除过度保护的数据覆盖防护，确保比分正确更新
- 验证API数据准确性：所有60场已完赛比分与FIFA.com一致
- 新增足球主题图标（浏览器标签页 + Windows桌面应用）
- 下载方式改为网盘链接（百度网盘 + 夸克网盘）
- 版本比较改为语义化对比，避免版本号降级误报
- 赛事页面默认按日期分组显示
- 新增淘汰赛数据实时同步：API有数据时自动更新对阵，启动时硬编码兜底
- 修正淘汰赛日期和已确认对阵（南非vs加拿大、巴西vs日本、荷兰vs摩洛哥等）
- 所有淘汰赛添加北京时间开球时间
