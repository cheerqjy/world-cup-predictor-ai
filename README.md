...
### v1.13.0 (2026-06-28)
- 修复淘汰赛数据严重错误：修正全部16场1/16决赛的球队分配、日期和时间
- 数据来源：worldcup26.ir API + 体彩官网交叉验证
- 1/16决赛完整对阵：南非vs加拿大、巴西vs日本、德国vs巴拉圭、荷兰vs摩洛哥等
- seed.js淘汰赛更新改为每次启动强制覆盖，避免数据不一致
- scraper.js修正API数据提取（使用game.id匹配比赛编号）
- 数据库直接更新，确保部署包包含正确数据

### v1.12.0 (2026-06-28)
- 修复历史记录方案二无赛果比对：快照保存时picks已有actual但betSlip2未结算的场景
- 回填betSlip2时立即结算，避免won字段为null
- 结算后持久化到DB，避免每次请求重复计算
- 新增未结算检测：betSlip2有won=null的已完成比赛也触发重新结算
